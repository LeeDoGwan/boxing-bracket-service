...
- Latest `mvn test` result: 411 passed, 0 failed, 0 errors, 1 CI-only MariaDB smoke test skipped locally.
...
- Historical Sprint 1 baseline: 47 frontend tests passed across 19 test files, ESLint passed, and the Vite production build passed. The current 99-test verification baseline is maintained in [Testing](testing.md).
...
- Authenticated Judge API calls for assigned-ring loading, score query, and submission.
...
- Assigned-ring loading with scheduled bout list and current bout selection.
- Bout start, round start, status update, and next-bout transition commands.
- Active assigned rings are loaded from the shared staff assignment API before scoped operations.
...
- Retry state and 15-second automatic refresh for the read-only operations status API.
...
- Bout numbers are generated by the server per tournament and are not manually entered or imported.
- After a bout starts or has scoring/result references, admin schedule edits and deletion are rejected.
...
- `GET /api/staff/assignments/rings`
